...
def add_simple_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """
    Створює прості технічні індикатори (функція для зворотної сумісності)
    """
    df = df.copy()
    
    # Якщо MultiIndex, спрощуємо до звичайних колонок
...
def add_advanced_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """
    Додає розширений набір технічних індикаторів
    """
    df = df.copy()
    indicators = TechnicalIndicators()
    ...
    # Індикатори об'єму (якщо Volume є)
    if 'Volume' in df.columns:
...
def get_indicator_signals(df: pd.DataFrame) -> pd.DataFrame:
    """
    Генерує торгові сигнали на основі технічних індикаторів
    """
    df = df.copy()
...
def calculate_indicator_statistics(df: pd.DataFrame) -> pd.DataFrame:
    """
    Розраховує статистику для технічних індикаторів
    """
